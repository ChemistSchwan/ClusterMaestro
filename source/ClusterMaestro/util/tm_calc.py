...
This module was taken from the ASE project and some functinalities were added. The ASE
python package is published under the LGPL license, just like the ClusterMaestro python package.

The ASE package can be found in:
    https://gitlab.com/ase/ase
and the documentation can be found in:
    https://wiki.fysik.dtu.dk/ase/index.html

Original Text for this python module within the ASE module:
    This module defines an ASE interface to Turbomole: http://www.turbomole.com/
...
def addDispersion():
    with open("control", "r") as source:
        with open("control_2", "w") as target:
            for line in source:
                if "disp" in line:
                    target.write("$disp3 bj\n")
                else:
                    target.write(line)
    os.rename("control_2", "control")
...
def execute(args, input_str=None, error_test=True, stdout_tofile=True):
...
class Turbomole(FileIOCalculator):
    ...
    def verify_parameters(self):
        ...
        if self.parameters["initial guess"] not in ["eht", "hcore"]:
            if not (
                isinstance(self.parameters["initial guess"], dict)
                and "use" in self.parameters["initial guess"].keys()
            ):
    ...
    def get_define_str(self):
        ...
        define_str_tpl = (
            ...
            "__single_atom_str____norb_str____dft_str____dsp_str____ri_str__"
            "__ex__"
        ...
        if type(params["excitation"]) == list:
            exStr = "ex\n{}\n*\n{}\n*\n*\n\n".format(
                params["excitation"][0], params["excitation"][1]
            )
        else:
            exStr = ""
        ...
        if params["scf energy convergence"]:
            ...
            conv = params["scf energy convergence"]
            scfiter_str += "scf\nconv\n" + str(int(conv)) + "\n\n"
        ...
        define_str = re.sub("__ex__", exStr, define_str)
    ...
    def initialize(self):
        ...
        params = self.parameters
        if params["dispersion correction damping"] == "BJ":
            addDispersion()
    ...
    def read_vibrational_spectrum(self, noproj=False):
        ...
        for line in lines:
            dictionary = {}
            regex = (
                r"^\s+(\d+)\s+(\S*)\s+([-+]?\d+\.\d*)" r"\s+(\d+\.\d*)\s+(\S+)\s+(\S+)"
            )
            match = re.search(regex, line)
            if match:
    ...
    def read_charges(self):
        ...
        epsfit_defined = (
            "esp fit" in self.parameters and self.parameters["esp fit"] is not None
        )
